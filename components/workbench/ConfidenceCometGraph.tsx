...
export const ConfidenceCometGraph: React.FC<ConfidenceCometGraphProps> = ({
    ...
    switch (verdict) {
      ...
      default:
        return {
          ...
          badgeBg: "bg-[#131720] text-[#E2C15C] border-[#D4AF37]/40",
          icon: <HelpCircle className="h-3 w-3 text-[#D4AF37]" />,
        };
    }
  };
  ...
  return (
    <div
      ref={containerRef}
      className="bg-[#0D1017]/95 border border-[#D4AF37]/25 rounded-xl shadow-2xl shadow-black/70 flex flex-col transition-all duration-300 relative overflow-hidden my-6"
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 border-b border-[#D4AF37]/15 bg-[#08090C]/80 backdrop-blur-sm z-10">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-bold text-[#F8F9FA] tracking-wide">
                FORENSIC CONFIDENCE TRAJECTORY
              </h3>
              <span className="text-[10px] font-mono px-2.5 py-0.5 rounded-full font-semibold uppercase border border-emerald-700/50 bg-emerald-950/60 text-emerald-300 flex items-center gap-1.5">
                ...
                <span>{isAnalyzing ? "CALCULATING..." : "STATUS: ANALYSIS COMPLETE"}</span>
              </span>
            </div>
            <p className="text-xs text-[#94A3B8] font-sans mt-0.5">
              Confidence progression across atomic claim verification
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 self-end sm:self-auto">
          {/* Telemetry Box */}
          <div className="flex items-center gap-3 text-xs font-mono text-[#94A3B8] px-3 py-1.5 rounded-lg bg-[#131720] border border-[#D4AF37]/20">
            <div>
              CLAIMS: <span className="text-[#E2C15C] font-bold">{mappedPoints.length}</span>
            </div>
          ...
          <button
            ...
            title={isPlaying ? "Pause Comet Animation" : "Play Comet Animation"}
            className="p-2 rounded-lg bg-[#131720] hover:bg-[#1C2230] text-[#E2C15C] border border-[#D4AF37]/20 transition-colors shadow-sm"
          >
          ...
          <button
            ...
            title="Replay Trajectory Animation"
            className="p-2 rounded-lg bg-[#131720] hover:bg-[#1C2230] text-[#D4AF37] border border-[#D4AF37]/30 transition-colors shadow-sm"
          >
      ...
      <div className="p-4 sm:p-6 w-full relative select-none">
        {isAnalyzing ? (
          <div className="h-64 flex flex-col items-center justify-center gap-3 text-[#94A3B8] font-mono text-xs">
            ...
            <p className="animate-pulse text-[#E2C15C]">
          ...
          <div className="w-full overflow-x-auto">
            ...
            >
              {/* Definitions */}
              <defs>
                {/* Gold Trajectory Gradient */}
                <linearGradient id="goldTrajectoryGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" stopColor="#D4AF37" />
                  <stop offset="50%" stopColor="#F5E4AB" />
                  <stop offset="100%" stopColor="#C49A31" />
                </linearGradient>
                ...
                return (
                  <g key={score} className="grid-row">
                    <line
                      ...
                      y2={y}
                      stroke={isBaseline ? "rgba(212, 175, 55, 0.25)" : "rgba(255, 255, 255, 0.05)"}
              ...
              <text
                x={width - padding.right + 8}
                y={getY(35) + 3}
                className="text-[9px] font-mono fill-rose-500/70 font-semibold"
              >
                LOW (35%)
              </text>
              ...
              {pathD && (
                <>
                  ...
                  {/* Primary Metallic Gold Path */}
                  <path
                    id={`confidenceTrajectoryPath-${replayKey}`}
                    d={pathD}
                    fill="none"
                    stroke="url(#goldTrajectoryGrad)"
                    strokeWidth={3}
                    strokeLinecap="round"
                    strokeLinejoin="round"
              ...
              {/* Traveling Luminous Comet Particle along Trajectory */}
              {pathD && isPlaying && (
                <g className="comet-group">
                  {/* Outer Comet Glow Aura */}
                  <circle r={8} fill="#D4AF37" opacity={0.35} filter="url(#cometGlowFilter)">
                    <animateMotion
                      dur="3.2s"
                  ...
                  {/* Mid Comet Body */}
                  <circle r={5} fill="#F5E4AB" opacity={0.8} filter="url(#cometGlowFilter)">
                    <animateMotion
                      dur="3.2s"
                      repeatCount="indefinite"
                      path={pathD}
                  ...
                  >
                    ...
                    <text
                      ...
                      className={`text-xs font-mono font-bold transition-colors ${
                        isHovered ? "fill-[#E2C15C]" : "fill-[#94A3B8]"
                      }`}
                    >
                      {coord.point.claimId}
                    </text>
        ...
        {hoveredPointIndex !== null && tooltipPos && mappedPoints[hoveredPointIndex] && (
          <div
            className="absolute z-30 pointer-events-none p-3.5 rounded-xl bg-[#08090C]/95 border border-[#D4AF37]/40 shadow-2xl shadow-black max-w-xs text-xs space-y-2 backdrop-blur-md animate-in fade-in zoom-in-95 duration-150"
            style={{
          ...
          >
            ...
            <div className="flex items-center justify-between text-[10px] font-mono bg-[#131720] p-2 rounded-lg border border-stone-800">
      ...
      <div className="p-3 border-t border-stone-800/80 bg-[#08090C]/90 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs font-mono text-[#94A3B8] z-10">
        <div className="flex items-center gap-2 text-[#E2C15C]">
          ...
          <span>Deterministic Confidence Trajectory: HIGH (90%) • MEDIUM (65%) • LOW (35%)</span>
        </div>
